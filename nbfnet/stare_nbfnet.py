...
from . import tasks, stare_layers


class StarENBFNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation, message_func="distmult", aggregate_func="pna",
                 ...
                 dependent=True, remove_one_hop=False, num_beam=10, path_topk=10, p=None):
        super(StarENBFNet, self).__init__()
        ...
        for i in range(len(self.dims) - 1):
            self.layers.append(stare_layers.StarEGeneralizedRelationalConv(self.dims[i], self.dims[i + 1], num_relation,
                                                                           self.dims[0], message_func, aggregate_func, layer_norm,
                                                                           activation, dependent, p))
    ...
    def remove_easy_edges(self, data, h_index, t_index, r_index=None):
        # TODO: this may be different for hyper edges
        ...
        data = copy.copy(data)
        data.hyper_edge_index = data.hyper_edge_index[:, mask]
        data.hyper_edge_type = data.hyper_edge_type[mask]
        return data
    ...
    def bellmanford(self, data, h_index, r_index, separate_grad=False, query_embedding=None,
                    # added by starE
                    initial_x=None, quals=None, rel_embed=None, qual_rel_embed=None):
        ...
        boundary = torch.zeros(batch_size, data.hyper_num_nodes, self.dims[0], device=h_index.device)
        # by the scatter operation we put query (relation) embeddings as init features of source (index) nodes
        boundary.scatter_add_(1, index.unsqueeze(1), query.unsqueeze(1))
        size = (data.hyper_num_nodes, data.hyper_num_nodes)
        edge_weight = torch.ones(data.hyper_edge_index.shape[1], device=h_index.device)
        ...
        for layer in self.layers:
            ...
            hidden = layer(layer_input, query, boundary, data.hyper_edge_index, data.hyper_edge_type, size, edge_weight,
                           # added by starE
                           x=initial_x, quals=quals, rel_embed=rel_embed, qual_rel_embed=qual_rel_embed)  # initial_x is not updated,
                           # while starE will always update it.
        ...
        node_query = query.unsqueeze(1).expand(-1, data.hyper_num_nodes, -1) # (batch_size, num_nodes, input_dim)
    ...
    def beam_search_distance(self, data, edge_grads, h_index, t_index, num_beam=10):
        ...
        num_nodes = data.hyper_num_nodes
        input = torch.full((num_nodes, num_beam), float("-inf"), device=h_index.device)
        input[h_index, 0] = 0
        edge_mask = data.hyper_edge_index[0, :] != t_index

        distances = []
        back_edges = []
        for edge_grad in edge_grads:
            # we don't allow any path goes out of t once it arrives at t
            node_in, node_out = data.hyper_edge_index[:, edge_mask]
            relation = data.hyper_edge_type[edge_mask]
