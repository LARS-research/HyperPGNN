...
def negative_sampling(data, batch, num_negative, strict=True, hr2t_dict=None, tr2h_dict=None):
    ...
    if strict:
        t_mask, h_mask = strict_negative_mask(data, batch, hr2t_dict, tr2h_dict)
    ...
    # this corresponds to the predict() in h_task.py (train part)
    if batch.shape[0] > 3:
        r_index = torch.cat((r_index, (batch[3:]).T), dim=1)
...
def all_negative(data, batch):
    ...
    # this corresponds to the predict() in h_task.py (test part)
    # Need to reconstruct this part for n-ary data
    if batch.shape[0] > 3:  # TODO: check use shape[0] or shape[1]?
        r_index = torch.cat((r_index, (batch[3:]).T), dim=1)
    ...
    t_batch = [h_index, t_index, r_index]
    # generate all negative heads for this batch
    all_index = torch.arange(data.num_nodes, device=batch.device)
    t_index, h_index = torch.meshgrid(pos_t_index, all_index)
    h_batch = [h_index, t_index, r_index]

    return t_batch, h_batch


def strict_negative_mask(data, batch, hr2t_dict=None, tr2h_dict=None):
    # TODO: modify this for the n-ary, need strict negative for n-ary data
    ...
    # batch = batch.t()  # not needed, because batch is transposed in negative_sampling/all_negative/test
    pos_h_index, pos_t_index, pos_r_index, pos_quals = batch[0], batch[1], batch[2], batch[3:]
    # pos_h_index, pos_t_index, pos_r_index = batch.t()
    ...
    if hr2t_dict is None:
        # search for all true tails for the given (h, r) batch
        edge_id, num_t_truth = edge_match(edge_index, query_index)
        # num_t_truth is #Batch_size
        # edge_idx is of #sum(num_t_truth) length
        ...
        t_mask.scatter_(1, pos_t_index.unsqueeze(-1), 0)  # this seem to make no difference
    else:
        t_mask = torch.ones(batch.shape[1], data.num_nodes, dtype=torch.bool, device=batch.device)
        for i, (h_i, r_i, quals_i) in enumerate(zip(pos_h_index, pos_r_index, pos_quals.t())):
            positive_t = hr2t_dict[(int(h_i), int(r_i), *quals_i.tolist())]
            t_mask[i, positive_t] = 0
    ...
    if tr2h_dict is None:
        ...
        h_mask = torch.ones(batch.shape[1], data.num_nodes, dtype=torch.bool, device=batch.device)
        for i, (t_i, r_i, quals_i) in enumerate(zip(pos_t_index, pos_r_index, pos_quals.t())):
            positive_h = tr2h_dict[(int(t_i), int(r_i), *quals_i.tolist())]
            h_mask[i, positive_h] = 0
