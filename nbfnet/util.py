...
from hbfnet.datasets import NAryLinkPrediction, NaryLinkPredictionInductive
from hbfnet.meta_datasets import MetaKnowledgeGraphDatasetCollection
from hbfnet.models import HyperBFNet
from hbfnet.meta_models import MetaHyperBFNet
...
def load_config(cfg_file, context=None):
    ...
    # prepare the config
    # cfg.model.nbf_config.version = cfg.model.version
    # cfg.model.hyper_relation_learner_config.version = cfg.model.version
    return cfg
...
def parse_args():
    ...
    parser.add_argument("-n", "--name", help="Manually set the name of this experiment, will effect the log directory"
                        , type=str, default="")
...
def create_working_directory(cfg):
    ...
    if cfg.experiment_name != "":
        working_dir = os.path.join(os.path.expanduser(cfg.output_dir),
                                   cfg.model["class"], cfg.dataset["class"], cfg.experiment_name)
    else:
        working_dir = os.path.join(os.path.expanduser(cfg.output_dir),
    ...
    if get_rank() == 0:
        ...
        try:
            ...
            # ask user do you want to overwrite the previous result
            answer = input("The working directory %s already exists, do you want to overwrite(o) it or continual(c) or quit(q)? (o/c/q) " % working_dir)
            if answer.lower() == "o":
                # remove the old directory working_dir using shutil
...
def build_nary_dataset(cfg):
    cls = cfg.dataset.pop("class")
    if cls == "JF17K_parse_HG":
        dataset = NAryLinkPrediction(root=cfg.dataset.path, name="jf17k_parse")  # name is used to construct the path
    elif cls == "WD50K_HG":
        dataset = NAryLinkPrediction(root=cfg.dataset.path, name="wd50k")
    elif cls == "WikiPeople_HG":
        dataset = NAryLinkPrediction(root=cfg.dataset.path, name="Wikipeople")
    elif cls == "JF-IND_inductive" or cls == "WP-IND_inductive" or cls == "MFB-IND_inductive":
        dataset = NaryLinkPredictionInductive(root=cfg.dataset.path, name=cls[:cls.find("_")])
    elif cls == "Meta_parse_HG":
        dataset = MetaKnowledgeGraphDatasetCollection(path=cfg.dataset.path, parameter=cfg.dataset.parameter)
    else:
        raise ValueError("Unknown dataset `%s`" % cls)
    ...
    #     logger.warning("%s dataset" % cls)
    #     logger.warning("#train: %d, #valid: %d, #test: %d" %
    #                    (len(dataset[0].statements), len(dataset[1].statements),
    #                     len(dataset[2].statements)))
...
def build_nary_model(cfg):
    cls = cfg.model.pop("class")
    if cls == "HBFNet":
        model = HyperBFNet(cfg.model.nbf_config, cfg.model.hyper_relation_learner_config, cfg.model.score_config)
    elif cls == "MetaHBFNet":
        model = MetaHyperBFNet(cfg.model.nbf_config, cfg.model.hyper_relation_learner_config,
                               cfg.model.score_config, cfg.model.meta_config, cfg.model.support_nbf_config)
    elif cls == "StarEHBFNet":
        model = HyperBFNet(cfg.model.starE_nbf_config, cfg.model.hyper_relation_learner_config,
                               cfg.model.score_config)
    else:
        raise ValueError("Unknown model `%s`" % cls)
    if "checkpoint" in cfg:
        state = torch.load(cfg.checkpoint, map_location="cpu")
