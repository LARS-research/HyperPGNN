# copy from my h_layer_meta.py, which allows relations in qualifiers use the same embedding as those in main relations
#  compared with h_layer.py.
...
from .utils import get_param, ccorr, rotate
...
class EdgeProcessing(MessagePassing):
    """ The important stuff. """
    ...
    def __init__(self, in_channels, out_channels, num_rels, act=lambda x: x,
        ...
        # self.w_loop = nn.Embedding(in_channels, out_channels)  # (100,200)
        # self.w_in = nn.Embedding(in_channels, out_channels)  # (100,200)
        # self.w_out = nn.Embedding(in_channels, out_channels)  # (100,200)
        # self.w_rel = nn.Embedding(in_channels, out_channels)  # (100,200)
    ...
    def forward(self, x, edge_index, edge_type, quals, rel_embed, qual_rel_embed=None):
        ...
        quals would be                  :   [qr1,qr2,qr1,qr2,qr3,qr2,qr1,qr4,qr2]
                                            [qe1,qe2,qe1,qe3,qe3,qe2,qe1,qe1,qe4]
                                            [  0,  0,  1,  1,  2,  2,  3,  5,  5]
        ...
        if self.p['STATEMENT_LEN'] != 3 and len(quals[1]) > 0:
            ...
            in_res = self.propagate(self.in_index,
                                    x=x,
            ...
            loop_res = self.propagate(self.loop_index,
                                      x=x,
            ...
            out_res = self.propagate(self.out_index,
                                     x=x,
